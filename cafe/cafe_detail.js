...
cafeName.innerHTML = sessionStorage.getItem("cafeNameKo");

// 저장되어있는 리뷰를 table에 추가해야함.
  var filename = "./cafe_review/" + sessionStorage.getItem('cafeName') + ".json";
  var jsonInfo = { fileName: filename};
  $.ajax({
    url: "cafe_detail_review.php",
    type: "POST",
    async: false,
    data: jsonInfo,
  ...
  // 위치 가져오기
  var jsonInfo = {
  fileName: "cafeList.json",
  cafeName: sessionStorage.getItem('cafeName')
  };
  // 해당하는 카페 이름의 위치 불러오기
  $.ajax({
  url: "cafe_detail_location.php",
  type: "POST",
  async: false,
  data: jsonInfo,
  success: function(data) { // 성공시 실행할 callback 함수
  ...
  // 이미지 사진 바꾸기
$("#cafeImg").attr("src" , sessionStorage.getItem('cafeName')+".jpg")
